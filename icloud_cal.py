...
# 일정 만들어 넣기
# ---------------------------------------------------------------------------
def alarm_lines(event, anchor, now=None):
    """3일 전 · 1일 전 · 당일(1시간 전) 알림을 **언제나 세 개 다** 붙입니다.

    울릴 시각을 정하는 규칙은 구독 캘린더(docs/law.ics)와 같습니다.
    build_ics.alarm_times() 가 그 규칙을 갖고 있으니 그대로 불러 씁니다.
    여기에 따로 적으면 손으로 넣은 일정과 공지에서 뽑은 일정의 알림 시각이
    달라집니다.
    ...
    지난 알림도 빼지 않는 이유
    -------------------------
    모레 일정을 넣으면 'D-3' 시각은 이미 지나 있습니다. 그걸 빼 버리면
    캘린더에 알림이 하나만 붙어서, 세 개를 기대한 사람이 "왜 안 붙었지"
    하게 됩니다. 애플 캘린더는 이미 지난 시각의 알림을 뒤늦게 울리지
    않으므로, 넣어 두어도 시끄러워지지 않습니다.

    (구독 캘린더 쪽은 사정이 다릅니다. 거기는 지난 일정이 수십 건 한꺼번에
     들어오므로 build_ics.py 가 지난 알림을 걸러 냅니다.)
    """
    lines = []
